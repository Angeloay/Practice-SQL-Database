...
-- first 5 employee limit doesn't work in MSSQL
SELECT TOP 5 *
FROM employee
--LIMIT 5;

-- first and last name as forename lastname
...
-- find  the number of employee has supervisor 
SELECT COUNT (super_id) AS have_supervisor
FROM employee

-- find number of employee female born after 1970
SELECT COUNT (emp_id) AS born_after_1970
FROM employee
...
-- flind the sum of all employee's salaries
...
--find any brnach supplier who are in the label business 
SELECT *
...
-- not as useful UNION
-- find all list of all clinets & branch suppliers
...
-- left join, we include all of the rows from the left table (FROM employee table),
